...
import { IgnorationService } from './ignoration.service';

describe('IgnorationService', () => {
  ...
  describe('create()', () => {
    it('creates and returns a Ignoration', () => {
      const params = {
      ...
      service.create(params).then(res => {
        expect(res).toEqual(jasmine.any(Ignoration));
  ...
  describe('find()', () => {
    it('returns an array of Ignorations', () => {
      const _id = chance.hash();
      const params = {
        where: { _id },
      };
      ...
      service.find(params).then(res => {
        ...
        expect(res[0]).toEqual(jasmine.any(Ignoration));
