...
export class GamesJsonPageComponent implements OnInit {
  ...
  public ngOnInit() {
    this.activatedRoute.paramMap.subscribe(async (params) => {
      ...
      this.breadcrumbs = [
        { label: 'Games', link: '../../' },
        { label: _id === 'new' ? 'Create Game' : 'Edit Game', link: '../' },
        { label: _id === 'new' ? 'Create Game as JSON' : 'Edit Game as JSON' },
      ];
  ...
  private async upsert(data: Partial<Game>) {
    ...
    if (this.data._id) {
      ...
      result = await this.gameService.update(data);
    } else {
      result = await this.gameService.create(data);
    }
