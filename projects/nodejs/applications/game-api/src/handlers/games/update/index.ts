...
export async function handler(ctx: Context) {
  const where = await GamePermissions.where({ name: ctx.params.name }, ctx.state.user);
  ...
  const result = await GamePermissions.update(record, ctx.request.body, {}, ctx.state.user);
