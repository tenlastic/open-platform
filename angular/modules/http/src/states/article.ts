...
export class ArticleQuery extends QueryEntity<ArticleState, Article> {
  constructor(protected namespaceQuery: NamespaceQuery, protected store: ArticleStore) {
    super(store);
  }

  public populate($input: Observable<Article[]>) {
    return combineLatest([$input, this.namespaceQuery.selectAll({ asObject: true })]).pipe(
      map(([articles, namespaces]) => {
