...
import { Storefront } from '@tenlastic/ng-http';
...
export class StatusComponent implements OnChanges, OnDestroy, OnInit {
  @Input() public storefront: Storefront;
  ...
  public ngOnInit() {
    if (this.updateService) {
      this.status = this.updateService.getStatus(this.storefront._id);
      this.updateService.checkForUpdates(this.storefront._id, true);
    }
  ...
  public click() {
    if (this.status.state === UpdateServiceState.Ready && this.status.childProcess) {
      this.updateService.stop(this.storefront._id);
    } else if (this.status.state === UpdateServiceState.Ready && !this.status.childProcess) {
      this.updateService.play(this.storefront._id);
    } else if (this.status.state === UpdateServiceState.NotInstalled) {
      this.updateService.install(this.storefront._id);
    }
  }

  public delete() {
    return this.updateService.delete(this.storefront._id);
  }

  public showInExplorer() {
    this.updateService.showInExplorer(this.storefront._id);
  }

  public sync() {
    const status = this.updateService.getStatus(this.storefront._id);
    status.state = UpdateServiceState.NotChecked;

    this.updateService.checkForUpdates(this.storefront._id, false);
  }
}
