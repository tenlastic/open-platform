...
describe('routes/namespaces/logs', function () {
  ...
  context('when permission is granted', function () {
    it('returns log records', async function () {
      ...
      const namespace = await NamespaceMock.create({
        status: { nodes: [{ _id: chance.hash(), phase: 'Running' }] } as any,
        users: [namespaceUser],
      });
    ...
    it('throws an error', async function () {
      const namespaceUser = NamespaceUserMock.create({ _id: user._id, roles: [] });
