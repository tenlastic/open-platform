...
export async function handler(ctx: Context) {
  ...
  const database = await DatabasePermissions.findOne({}, override, user);
  if (!database) {
    throw new RecordNotFoundError('Record');
  }

  // Check if the record contains the requested node.
  const node = database.status?.nodes?.find(n => n._id === ctx.params.nodeId);
  if (!node) {
    throw new RecordNotFoundError('Record');
  }

  // Check if the user can access the record's logs.
  const permissions = DatabasePermissions.accessControl.getFieldPermissions('read', database, user);
