...
describe('web-server/steam-api-keys/create', function () {
  ...
  context('when permission is granted', function () {
    ...
    beforeEach(async function () {
      ...
      await AuthorizationModel.mock({
        ...
        roles: [AuthorizationRole.SteamApiKeysRead, AuthorizationRole.SteamApiKeysWrite],
        userId: user._id,
      }).save();
    });

    it('creates a Steam API Key', async function () {
      const appId = chance.integer();
      const ctx = new ContextMock({
        params: { namespaceId: namespace._id },
        request: { body: { appId, name: chance.hash(), value: chance.hash() } },
        state: { user },
      } as any);

      const app = { app_name: chance.hash(), app_type: chance.hash(), appid: appId };
      sandbox
  ...
  context('when the value is invalid', function () {
    it('throws an error', async function () {
      ...
      const ctx = new ContextMock({
        params: { namespaceId: namespace._id },
        request: { body: { appId: chance.integer(), value: chance.hash() } },
        state: { user },
      } as any);
  ...
  context('when the appId is invalid', function () {
    it('throws an error', async function () {
      const namespace = await NamespaceModel.mock({ limits: NamespaceLimitsModel.mock() }).save();
  ...
  context('when permission is denied', function () {
    it('throws an error', async function () {
      ...
      const ctx = new ContextMock({
        params: { namespaceId: namespace._id },
        request: { body: { appId, value: chance.hash() } },
        state: { user },
      } as any);
