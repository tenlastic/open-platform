...
describe('builds', function () {
  ...
  step('does not create a build', async function () {
    ...
    const promise = buildService.create(
      {
    ...
    return expect(promise).to.be.rejectedWith('BuildModule');
  });

  it.only('creates a Build Module', async function () {
    // Create the Build Module.
    let module: BuildModuleModel = await moduleService.create({
      namespaceId: namespace._id,
      type: BuildModuleModel.discriminatorValue,
    });
    expect(module).to.exist;

    // Wait for Module to start running.
    return wait(1000, 180000, async () => {
      module = await moduleService.findOne(module._id);
      return module.status?.phase === 'Running';
    });
  });
